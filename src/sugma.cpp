...
int main()
{
    ...
    unsigned char *data = stbi_load("..\\..\\src\\shader_texture_stuff\\container.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("..\\..\\src\\shader_texture_stuff\\BillIcon.png", &width, &height, &nrChannels, 0);
    if (data)
    {
...
int main()
{
    ...
    // glfw window creation
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "*Physics Engine* (not yet though)", NULL, NULL);
    if (window == NULL)
    {
        std::cout << "Failed to create GLFW window" << std::endl;
        glfwTerminate();
